...
private:
  ...
  ros::Subscriber ttargetposeSub_, targetexistSub_,arucopose_,simple_goal_sub;
  ...
  double holdTargetPos_x_, holdTargetPos_y_, holdTargetPos_z_,holdAtt_x,holdAtt_y,holdAtt_z,holdAtt_w;
  bool first_enter_hold = true;
  ...
  double mavYaw_;
  Eigen::Vector3d simple_goal;
  Eigen::Vector3d aruco_cmd_,Karuco_;
  Eigen::Vector3d g_;
  ...
  double Kpos_x_, Kpos_y_, Kpos_z_, Kvel_x_, Kvel_y_, Kvel_z_,Karuco_angle,Karuco_velx,Karuco_vely;
  ...
  void arucoCallback(const std_msgs::Float32MultiArray &msg);
  void
  multiDOFJointCallback(const trajectory_msgs::MultiDOFJointTrajectory &msg);
  void movebaseCallback(const geometry_msgs::PoseStampedPtr &msg);
  ...
  void move_base_simple(const geometry_msgs::PoseStampedPtr &msg);

  Eigen::Vector3d computeAruco_vel(Eigen::Vector3d &aruco_cmd);
